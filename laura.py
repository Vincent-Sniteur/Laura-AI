...
class ChatGPT:
    '''
    Laura AI ChatGPT class for interacting with the OpenAI ChatGPT API.
    '''
    ...
    def __init__(
        ...
        if self.__auth_type not in [None]:
    ...
    def __init_logger(self, verbose: bool) -> None:
        ...
        '''
        self.logger = logging.getLogger('LauraAI')
    ...
    def __check_blocking_elements(self) -> None:
        '''
        Check for all blocking elements and dismiss them
        '''
        self.logger.debug('Checking for blocking elements...')
        try:
        ...
        if alerts:
            self.logger.debug('Dismissing alerts...')
...
#  Send a message to ChatGPT and return the response
    def send_message(self, message: str, stream: bool = False) -> dict:
        ...
        self.logger.debug('Attempting to get response...')
        responses = self.driver.find_elements(*chatgpt_big_response)
        if responses:
            response = responses[-1]
            if 'text-red' in response.get_attribute('class'):
    ...
    def reset_conversation(self) -> None:
        '''
        Reset the conversation on the chat page
        '''
    ...
    def clear_conversations(self) -> None:
        '''
        Clear all conversations from the chat page
        '''
        if not self.driver.current_url.startswith(chatgpt_chat_url):
    ...
    def refresh_chat_page(self) -> None:
        '''
        Refresh the chat page & check for capacity and blocking elements
        '''
        if not self.driver.current_url.startswith(chatgpt_chat_url):
